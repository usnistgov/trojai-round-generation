...
class GrainMerge(ImageMerge):
    ...
    def do(self, img_obj: ImageEntity, pattern_obj: ImageEntity, random_state_obj: RandomState) -> ImageEntity:
        ...
        if semantic is not None:
            # blank out the semantics under the pattern
            semantic[pattern_mask] = 0
        return GenericImageEntity(img_out, mask_out, semantic)
...
class AddMerge(ImageMerge):
    ...
    def do(self, img_obj: ImageEntity, pattern_obj: ImageEntity, random_state_obj: RandomState) -> ImageEntity:
        ...
        if semantic is not None:
            # blank out the semantics under the pattern
            semantic[pattern_obj.get_mask()] = 0
        return GenericImageEntity(img_out, mask_out, semantic)
...
class GrainMergePaste(ImageMerge):
    ...
    def do(self, img_obj: ImageEntity, pattern_obj: ImageEntity, random_state_obj: RandomState) -> ImageEntity:
        ...
        if semantic is not None:
            # blank out the semantics under the pattern
            semantic[pattern[:,:,3] > 0] = 0

        return GenericImageEntity(final_img, final_mask, semantic)

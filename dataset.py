...
def build_image(config: round_config.RoundConfig, rso: np.random.RandomState, fg_image_fp: str, bg_image_fp: str, obj_class_label: int, allow_spurious_triggers: bool = True) -> (np.ndarray, int, int, bool):
    ...
    np_img = processed_img.get_data().astype(config.img_type)
...
class TrafficDataset:
    ...
    """
    def __init__(self, config: round_config.RoundConfig, random_state_obj: np.random.RandomState, num_samples_to_generate: int, class_balanced: bool = True, worker_process_count: int = None):
        """

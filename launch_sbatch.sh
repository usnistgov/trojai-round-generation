...
MODEL_DIR=/mnt/isgnas/home/mmajursk/trojai/r10/models
NJOBS=48
MODELS_PER_JOB=1000
...
do
  ...
  then
      ...
      echo "Executing (heimdall): $i with start: $START_RUN with length: $MODELS_PER_JOB"
      sbatch sbatch_script_heimdall.sh $START_RUN $MODELS_PER_JOB
  else
      START_RUN=$((START_INDEX + (i-1) * $MODELS_PER_JOB))

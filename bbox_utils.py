...
def xywh_to_xyxy_format(x):
    """
    Converts a numpy array of bounding boxes of center format (upper_left_x, upper_left_y, width, height) to corners format
    (x_0, y_0, x_1, y_1).
    """
    if len(x.shape) == 1:
        x = x.reshape(1, -1)
...
def draw_boxes(img, boxes, value=0):
    ...
    for i in range(boxes.shape[0]):
        x_st = max(0, round(boxes[i, 0]))
        y_st = max(0, round(boxes[i, 1]))

        x_end = min(img.shape[1], round(boxes[i, 2]))
        y_end = min(img.shape[0], round(boxes[i, 3]))
...
def compute_iou(box, boxes, box_area=None, boxes_area=None):
    if box.shape[0] == 0 or boxes.shape[0] == 0:
        return np.zeros((0))
    box = box.reshape(-1)  # make the box a vector if its a matrix of [1,4]
...
def compute_intersection(box, boxes):
    ...
    intersections = np.maximum(y_bottom - y_top, 0) * np.maximum(x_right - x_left, 0)
    boxes_area = (boxes[:, 2] - boxes[:, 0]) * (boxes[:, 3] - boxes[:, 1])
    return intersections, boxes_area

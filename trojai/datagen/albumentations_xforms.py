...
from trojai.datagen.albumentations import fog, rain
...
class BrightenXForm(ImageTransform):
    ...
    def do(self, input_obj: ImageEntity, random_state_obj: RandomState) -> ImageEntity:
        ...
        return GenericImageEntity(img_xformed, input_obj.get_mask(), input_obj.get_semantic_label_mask())
...
class AddShadowXForm(ImageTransform):
    ...
    def __init__(self, no_of_shadows: int = 1, rectangular_roi: tuple = (-1, -1, -1, -1),
                 shadow_dimension: int = 5, pre_normalize: bool = True, post_normalize: bool = True, probability=1.0) -> None:
        """
    ...
    def do(self, input_obj: ImageEntity, random_state_obj: RandomState) -> ImageEntity:
        ...
        :return: the transformed Entity
        """
        if random_state_obj.random() < self.probability:
            img = input_obj.get_data()
...
class AddRainXForm(ImageTransform):
    ...
    def __init__(self, slant: float = -1, drop_length: float = 20, drop_width: float = 1,
                 ...
                 post_normalize: bool = True, probability: float = 0.5):
        """
        ...
        self.post_normalize = post_normalize
        self.probability = probability
        # the probability is calculated during the do, not the call to the rain_object. So it needs to always fire when called
        self.rain_object = rain.RandomRain(
            slant_lower, slant_upper, self.drop_length, self.drop_width,
            self.drop_color, blur_value=7, brightness_coefficient=0.7,
            rain_type=self.rain_type, always_apply=True)
    ...
    def do(self, input_obj: ImageEntity, random_state_obj: RandomState) -> ImageEntity:
        ...
        if random_state_obj.random() < self.probability:
            ...
            self.rain_object.random_state_obj = random_state_obj
            rgb_img_xformed = self.rain_object(image=rgb_img)['image']
            img_xformed = normalization_from_rgb(rgb_img_xformed, alpha_ch, self.post_normalize, original_n_chan,
                                                 "AddRainXForm")
...
class AddSnowXForm(ImageTransform):
    ...
    """
    def __init__(self, snow_coeff: float = -1, pre_normalize: bool = True, post_normalize: bool = True, probability: float =0.5):
        """
        ...
        # the probability is calculated during the do, not the call to the snow_object. So it needs to always fire when called
        self.snow_object = albu.RandomSnow(snow_coeff_lower, snow_coeff_upper, brightness_coeff=2.5, always_apply=True)
    ...
    def do(self, input_obj: ImageEntity, random_state_obj: RandomState) -> ImageEntity:
        ...
        if random_state_obj.random() < self.probability:
            ...
            img_xformed = normalization_from_rgb(rgb_img_xformed, alpha_ch, self.post_normalize, original_n_chan, "AddSnowXForm")
            return GenericImageEntity(img_xformed, input_obj.get_mask(), input_obj.get_semantic_label_mask())
        else:
            return input_obj
...
class AddFogXForm(ImageTransform):
    ...
    """
    def __init__(self, fog_coeff: float = -1, pre_normalize: bool = True, post_normalize: bool = True, probability: float = 0.5):
        """
        Initializes the fog adder
        :param fog_coeff: coefficient between 0 and 1 controlling the amount of fog,
        if fog_coeff is -1, the value is drawn from the uniform distribution on [0.3, 0.8) upon each application`
        :param pre_normalize: whether to automatically convert input to this transform's required channel format (RGB)
        :param post_normalize: whether to automatically convert output back to its original channel format
        :param probability: probability of the transformation being applied
        """
        ...
        if fog_coeff == -1:
            fog_coeff_lower, fog_coeff_upper = 0.3, 0.8
        self.pre_normalize = pre_normalize
        self.post_normalize = post_normalize
        self.probability = probability

        # the probability is calculated during the do, not the call to the fog_object. So it needs to always fire when called
        self.fog_object = fog.RandomFog(fog_coeff_lower, fog_coeff_upper, alpha_coef=0.08, always_apply=True)
    ...
    def do(self, input_obj: ImageEntity, random_state_obj: RandomState) -> ImageEntity:
        ...
        if random_state_obj.random() < self.probability:
            ...
            self.fog_object.random_state_obj = random_state_obj
            rgb_img_xformed = self.fog_object(image=rgb_img)['image']
            img_xformed = normalization_from_rgb(rgb_img_xformed, alpha_ch, self.post_normalize, original_n_chan, "AddFogXForm")
            return GenericImageEntity(img_xformed, input_obj.get_mask(), input_obj.get_semantic_label_mask())
        else:
            return input_obj
